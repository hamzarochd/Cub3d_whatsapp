...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/26 17:56:44 by hrochd            #+#    #+#             */
/*   Updated: 2025/04/26 18:16:03 by hrochd           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_wall(t_mlx *mlx, char direction)
{
	...
	if (handle_wall_collision(mlx, x, y))
		return (1);
	if (old_x / TILE_SIZE != x / TILE_SIZE
		&& old_y / TILE_SIZE != y / TILE_SIZE)
		return (check_diagonal(mlx->cube->map,
				x / TILE_SIZE,
				y / TILE_SIZE, mlx));
	return (0);
}
